...
if (isset($_POST['submit'])) {
    ...
    $query = "INSERT INTO makanan (nama_makanan, deskripsi, harga) VALUES (?, ?, ?)";

    if ($stmt = $conn->prepare($query)) {
        $stmt->bind_param("ssi", $nama_makanan, $deskripsi, $harga);
...
<form action="" method="post" autocomplete="off">
    ...
    <input type="text" id="makanan" name="nama_makanan" placeholder="Masukkan nama makanan" required>
    
    <label for="deskripsi">Deskripsi Makanan : </label>
    <input type="text" name="deskripsi" id="deskripsi" placeholder="Masukkan deskripsi makanan" required>
    
    <label for="harga">Harga Makanan : </label>
    <input type="number" id="harga" name="harga" placeholder="Masukkan harga makanan" required>
